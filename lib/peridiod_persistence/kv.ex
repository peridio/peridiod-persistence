defmodule PeridiodPersistence.KV do
  @moduledoc """
  ...
  To change the KV backend, implement the `PeridiodPersistence.KVBackend` behaviour and
  ...
  Getting all firmware metadata:
      ...
      iex> PeridiodPersistence.KV.get_all()
      %{
  ...
  currently active firmware slot:
      ...
      iex> PeridiodPersistence.KV.get_all_active()
      %{
  ...
  entire key name, while `get_active/1` will prepend the slot prefix for you:
      ...
      iex> PeridiodPersistence.KV.get("peridio_active")
      "b"
      iex> PeridiodPersistence.KV.get("b.peridio_uuid")
      "4e08ad59-fa3c-5498-4a58-179b43cc1a25"
      iex> PeridiodPersistence.KV.get_active("peridio_uuid")
      "4e08ad59-fa3c-5498-4a58-179b43cc1a25"
  ...
  in which case they will overwrite the current value with that key:
      ...
      iex> :ok = PeridiodPersistence.KV.put("my_firmware_key", "my_value")
      iex> :ok = PeridiodPersistence.KV.put("peridio_serial_number", "my_new_serial_number")
      iex> PeridiodPersistence.KV.get("my_firmware_key")
      "my_value"
      iex> PeridiodPersistence.KV.get("peridio_serial_number")
      "my_new_serial_number"

  It is possible to write a collection of values at once, in order to
  minimize number of writes:

      iex> :ok = PeridiodPersistence.KV.put_map(%{"one_key" => "one_val", "two_key" => "two_val"})
      iex> PeridiodPersistence.KV.get("one_key")
      "one_val"
  ...
  currently active firmware slot without specifying the slot prefix yourself:
      ...
      iex> :ok = PeridiodPersistence.KV.put_active("peridio_misc", "Peridio is awesome")
      iex> PeridiodPersistence.KV.get_active("peridio_misc")
      "Peridio is awesome"
  """
  use GenServer
  ...
  * `:kv_backend` - a KV backend of the form `{module, options}` or just `module`
  ...
  defp initial_state(options) do
    case options[:kv_backend] do
      ...
      _ ->
        initialize(PeridiodPersistence.KVBackend.UBootEnv, [])
    end
  rescue
    error ->
      Logger.error("Peridiod has a bad KV configuration: #{inspect(error)}")
      initialize(PeridiodPersistence.KVBackend.InMemory, [])
  end
